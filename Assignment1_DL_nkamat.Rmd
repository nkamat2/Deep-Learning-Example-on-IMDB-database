...
4. Using tanh activation function:
```{r}
model <- keras_model_sequential() %>% 
...
5.a. Using Dropout Function:
```{r}
model <- keras_model_sequential() %>% 
  layer_dense(units = 16, activation = "relu", input_shape = c(10000)) %>%
  layer_dropout(rate = 0.2) %>%  #Output values scaled down by a factor of 0.2
  layer_dense(units = 16, activation = "relu") %>% 
  layer_dropout(rate = 0.2) %>%  #Output values scaled down by a factor of 0.2
  layer_dense(units = 1, activation = "sigmoid")
model %>% compile(
...
model %>% fit(partial_x_train, partial_y_train, epochs = 4, batch_size = 512, validation_data = list(x_val, y_val))
results <- model %>% evaluate(x_test, y_test)
...
model %>% predict(x_test[1:10,])
```
5.b. Using Regularization Function:
```{r}
model <- keras_model_sequential() %>% 
  layer_dense(units = 16, kernel_regularizer = regularizer_l2(0.0001), #L2 weight regularization applied with square of weight coefficient as 0.0001
...
5.d. Best combination of Regularization and Dropout Function for highest Validation accuracy:
```{r}
model <- keras_model_sequential() %>% 
  layer_dense(units = 16, kernel_regularizer = regularizer_l2(0.0001), #L2 weight regularization applied with square of weight coefficient as 0.0001
              activation = "relu", input_shape = c(10000)) %>% 
  layer_dropout(rate = 0.2) %>%  #Output values scaled down by a factor of 0.25
  layer_dense(units = 16, kernel_regularizer = regularizer_l2(0.0001), #L2 weight regularization applied with square of weight coefficient as 0.0001
              activation = "relu") %>% 
  layer_dropout(rate = 0.2) %>%  #Output values scaled down by a factor of 0.25
  layer_dense(units = 1, activation = "sigmoid")
